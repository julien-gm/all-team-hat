...
public class Team
{
	...
	public double getSportScore(List<Double> pExpectedScores)
	{
		...
		return getScore(expectedSportScore, score);
	}
	...
	public double getStandardDeviation(List<Double> pExpectedScores)
	{
		...
		return this.players.stream()
			.mapToDouble(player -> Math.abs(teamSportAverage
				- player.getSportScores(pExpectedScores).stream().mapToDouble(Double::doubleValue).sum()))
			.average().orElse(0);
	}
}
